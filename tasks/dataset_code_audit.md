...
| Stage | File/function | Main input fields | Main output fields | Notes |
|---|---|---|---|---|
| parquet row | `examples/data_preprocess/prepare.py` or env-specific data setup | `prompt`, `data_source`, `extra_info`, optional `images` | persisted `train_files` / `val_files` rows | For Sokoban, the board state is generated by env reset, not stored in parquet. |
| dataset item | `verl/utils/dataset/rl_dataset.py::RLHFDataset.__getitem__` | row prompt and metadata | `input_ids`, `attention_mask`, `position_ids`, `raw_prompt_ids`, `data_source`, `extra_info` | Applies chat template and tokenization. |
| collated batch | `verl/utils/dataset/rl_dataset.py::collate_fn` | list of dataset items | tensor stack plus numpy object metadata | Non-tensor metadata must be numpy arrays for `DataProto`. |
| DataProto | `verl/protocol.py::DataProto.from_single_dict` | collated dict | `batch`, `non_tensor_batch`, `meta_info` | Central container passed through trainer and rollout. |
| gen batch | `RayPPOTrainer.fit` | prompt `DataProto` | prompt tensors and metadata popped from batch | Passed into `TrajectoryCollector`. |
| rollout step | `TrajectoryCollector` + `vLLMRollout.generate_sequences` | current env prompt tensors | `prompts`, `responses`, `input_ids`, `attention_mask`, `position_ids`, optional `rollout_log_probs` | One sample per active env step. |
| env step | `EnvManager.step` | decoded response text | `next_obs`, `reward`, `done`, `info` | Projection creates `projected_action` and `is_action_valid`. |
| flattened train batch | `gather_rollout_data` | `total_batch_list`, `episode_rewards`, `episode_lengths` | step-level training `DataProto` | Nested trajectory order is flattened for RL update. |
| loss batch | `RayPPOTrainer.fit` | rollout batch | `response_mask`, `old_log_probs`, `ref_log_prob`, `token_level_rewards`, `advantages` | Actor loss is response-token masked. |
